...
template <class T> void changelength1d(T *&a, int oldleng, int newleng) {
...
// sequential search
template <typename Key, typename value> struct Node_of_seq {
  Key key;
  value val;
...
template <typename Key, typename value> class sequential_search {
private:
...
public:
  ...
  sequential_search(Node_of_seq<Key, value> *node) { this->first = node; };
  ...
  void set_compare(int (*comp)(const Key &, const Key &)) { compare = comp; }

  bool contains(Key k) { return get(k) != NULL; }
  ...
  void delete_key(Key key) { this->first = delete_key(this->first, key); }
...
template <class Key, class value> class binarysearch {
private:
  int N;
  Key *key;
  value *val;
...
public:
  ...
  Key *floor(Key the_key) {
    ...
    int i = rank(the_key);
    if (i < N && compare(the_key, key[i]) == 0) {
      return key + i;
    }
    if (i == 0) {
      return NULL;
...
// bst
template <typename Key, typename value> struct BST_Node {
  Key key;
  value val;
  BST_Node *left, *right;
  int N;
...
private:
  ...
  BST_Node<Key, value> *put(BST_Node<Key, value> *node, Key k, value v) {
    if (node == NULL)
      return new BST_Node<Key, value>(k, v, 1);
  ...
  BST_Node<Key, value> *select(BST_Node<Key, value> *thnode, int k) {
    ...
    if (cmp > k)
      return select(thnode->left, k);
    else if (cmp < k)
      return select(thnode->right, k - cmp - 1);
    else
      return thnode;
  }
  ...
  int rank(Key k, BST_Node<Key, value> *thenode) {
    ...
    if (cmp < 0)
      return rank(k, thenode->left);
    else if (cmp > 0)
      return rank(k, thenode->right) + size(thenode->left) + 1;
    else
      return size(thenode->left);
  }
...
public:
  ...
  BST_Node<Key, value> *max(BST_Node<Key, value> *thnode) {
    if (thnode->right == NULL)
      return thnode;
    else
      return max(thnode->right);
  }
  ...
  int rank(Key k) { return rank(k, root); }

  void delete_key(Key key) { root = delete_key(root, key); }
...
/*red-black-tree*/
template <typename Key, typename Value> struct R_B_Node {
  Key key;
  Value val;
...
private:
  ...
  R_B_Node<Key, Value> *put(R_B_Node<Key, Value> *h, Key key, Value val) {
    if (h == nullptr)
      return new R_B_Node<Key, Value>(key, val, RED, 1);
  ...
  R_B_Node<Key, Value> *delete_key(R_B_Node<Key, Value> *h, Key key) {
    if (compare(key, h->key) < 0) {
      ...
      if (compare(key, h->key) == 0) {
        ...
        h->right = delete_key(h->right, key);
      }
    }
    return balance(h);
  }
...
template <typename Key, typename Value> class separate_chain_hash {
private:
  int cap = 4;
  int n;
  int m;
  ...
  int hash_this(Key k) { return (int)(hash_of_Key(k) % m); }
...
public:
  ...
  void set_hash(size_t (*hash)(const Key &)) { hash_of_Key = hash; }
...
template <typename Key> size_t hash_std(const Key &s1) {
